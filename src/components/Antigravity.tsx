...
/** Puntos en porcentaje 0-100 (ej. los que forman "NutriData"). Si se pasan, las partículas usan estas posiciones. */
export type TextPoint = { x: number; y: number };

interface AntigravityInnerProps {
  /** Posiciones iniciales en % 0-100 (ej. getNutriDataTextPoints()). Si se pasan, count se ignora. */
...
const AntigravityInner = ({
  ...
  const numParticles = textPoints ? textPoints.length : count;
  const particles = useMemo(() => {
    ...
    for (let i = 0; i < numParticles; i++) {
      ...
      let x: number;
      let y: number;
      if (textPoints && textPoints[i]) {
        // Puntos del texto en % 0-100 → viewport (centro 0,0)
        x = (textPoints[i].x / 100 - 0.5) * width;
        y = (textPoints[i].y / 100 - 0.5) * height;
      } else {
    ...
    return temp;
  }, [numParticles, viewport.width, viewport.height, textPoints]);
